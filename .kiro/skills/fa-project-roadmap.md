...
- [x] 语义缓存
  - SemanticCacheAdvisor 用 Embedding 向量做语义相似度匹配
  - 相似度 >= 0.92 命中缓存，直接返回不调模型
  - 订单查询类不缓存（结果动态变化）
  - 最大缓存 500 条，超出自动淘汰一半
  - 缓存统计接口：GET /admin/cache-stats
  - 前端返回 cached=true/false 标识是否命中缓存

- [x] 多租户隔离
  - TenantContext（ThreadLocal）+ TenantInterceptor 自动提取租户ID
  - 支持 Header（X-Tenant-Id）和参数（tenantId）两种方式传递
  - 所有数据表加 tenant_id 字段，查询自动过滤
  - 对话记录、知识库、Token 用量、摘要全部按租户隔离
  - 不同商家的数据完全独立，互不可见
...
- [x] WebSocket 实时对话
  ...
  - 流式输出：模型每生成一个 token 实时推送到前端
  - 自动重连：断线 3 秒后自动重连
  - 集成意图识别 + 语义缓存 + 多租户
  - 前端页面：http://localhost:8080/ws-chat.html
  - 比 HTTP SSE 方案更适合双向实时通信
